export interface FinalPhotoMetadata {
  caption: string;
  alt: string;
}

export const finalPhotoMetadata: Record<string, FinalPhotoMetadata> = {
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-1-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 1.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-2-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 2.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-3-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 3.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-4-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 4.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-5-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 5.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-6-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 6.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-7-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 7.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "chuck-hutton-toyota-memphis-tn-chargepoint-express-plus-8-04-24.jpg": {
    caption:
      "ChargePoint Express Plus installation at Chuck Hutton Toyota in Memphis — project photo 8.",
    alt:
      "ChargePoint Express Plus DC fast charging installation at Chuck Hutton Toyota in Memphis, Tennessee.",
  },
  "denver-co-commercial-building-site-csi-sustainable-design.jpg": {
    caption: "Commercial building sustainable design site in Denver, Colorado.",
    alt:
      "Commercial building site in Denver, Colorado associated with sustainable design planning.",
  },
  "footprint-helene-asheville-nc-atmospheric-water-generator-10-24.jpg": {
    caption:
      "Atmospheric water generator supporting Hurricane Helene relief at an Asheville community housing development.",
    alt:
      "Atmospheric water generator deployed at a community housing development in Asheville, North Carolina after Hurricane Helene.",
  },
  "footprint-project-hurricane-helene-microgrid-camp-miller-2-10-24.jpg": {
    caption:
      "Solar-battery microgrid supporting Hurricane Helene relief operations at Camp Miller in Pensacola, North Carolina.",
    alt:
      "Solar and battery microgrid providing temporary power at Camp Miller in Pensacola, North Carolina after Hurricane Helene.",
  },
  "footprint-project-hurricane-helene-microgrid-camp-miller-3-10-24.jpg": {
    caption:
      "Solar-battery microgrid supporting Hurricane Helene relief operations at Camp Miller in Pensacola, North Carolina.",
    alt:
      "Solar and battery microgrid providing temporary power at Camp Miller in Pensacola, North Carolina after Hurricane Helene.",
  },
  "footprint-project-hurricane-helene-relief-barnardsville-1-10-24.jpg": {
    caption:
      "Hurricane Helene relief work in Barnardsville, North Carolina — project photo 1.",
    alt:
      "Hurricane Helene relief operations with FootPrint Project in Barnardsville, North Carolina.",
  },
  "footprint-project-hurricane-helene-relief-barnardsville-2-10-24.jpg": {
    caption:
      "Hurricane Helene relief work in Barnardsville, North Carolina — project photo 2.",
    alt:
      "Hurricane Helene relief operations with FootPrint Project in Barnardsville, North Carolina.",
  },
  "footprint-project-hurricane-helene-relief-camp-miller-1-10-24.jpg": {
    caption:
      "Hurricane Helene relief infrastructure at Camp Miller in Pensacola, North Carolina.",
    alt:
      "Temporary Hurricane Helene relief infrastructure at Camp Miller in Pensacola, North Carolina.",
  },
  "footprint-project-hurricane-helene-relief-free-solar-big-ivy-10-24.jpg": {
    caption:
      "Solar power support for Hurricane Helene relief in Big Ivy, North Carolina.",
    alt:
      "Portable solar power equipment supporting Hurricane Helene relief in Big Ivy, North Carolina.",
  },
  "footprint-project-hurricane-helene-relief-mandala-springs-nc-10-24.jpg": {
    caption: "Hurricane Helene relief support at Mandala Springs, North Carolina.",
    alt:
      "FootPrint Project Hurricane Helene relief support at Mandala Springs in North Carolina.",
  },
  "kia-chattanooga-tn-abb-dc-fast-charger-service-10-22.jpg": {
    caption: "ABB DC fast charger service at Kia in Chattanooga, Tennessee.",
    alt:
      "Technician servicing an ABB DC fast charger at a Kia site in Chattanooga, Tennessee.",
  },
  "lexus-greenville-sc-abb-dcfc-charger-install-1-02-23.jpg": {
    caption:
      "ABB DC fast charger installation at Lexus Greenville — project photo 1.",
    alt: "ABB DC fast charger installed at Lexus Greenville in South Carolina.",
  },
  "lexus-greenville-sc-abb-dcfc-charger-install-2-02-23.jpg": {
    caption:
      "ABB DC fast charger installation at Lexus Greenville — project photo 2.",
    alt: "ABB DC fast charger installed at Lexus Greenville in South Carolina.",
  },
  "lexus-greenville-sc-chargepoint-ct4000-pedestal-install-2-02-23.jpg": {
    caption: "ChargePoint CT4000 pedestal installation at Lexus Greenville.",
    alt:
      "ChargePoint CT4000 Level 2 EV charger installed at Lexus Greenville in South Carolina.",
  },
  "lexus-greenville-sc-chargepoint-ct4020-pedestal-install-1-02-23.jpg": {
    caption: "ChargePoint CT4020 pedestal installation at Lexus of Greenville.",
    alt:
      "ChargePoint CT4020 dual-output Level 2 EV charger installed at Lexus of Greenville in South Carolina.",
  },
  "lexus-huntsville-al-chargepoint-level2-install-01-23.jpg": {
    caption: "ChargePoint Level 2 EV charger installation at Lexus Huntsville.",
    alt:
      "ChargePoint Level 2 EV charger installed at Lexus in Huntsville, Alabama.",
  },
  "manheim-mt-juliet-tn-ev-charger-bank-install-1-09-23.jpg": {
    caption: "EV charger bank installation at Manheim Nashville.",
    alt:
      "Commercial EV charging bank installed at Manheim in Mt. Juliet, Tennessee.",
  },
  "nashville-tn-off-grid-battery-service-08-23.jpg": {
    caption: "Off-grid battery system service in Nashville, Tennessee.",
    alt:
      "Technician servicing an off-grid battery power system in Nashville, Tennessee.",
  },
  "range-rover-knoxville-tn-cpe250-dc-fast-charger-service-1-03-25.jpg": {
    caption:
      "ChargePoint Express 250 service at Land Rover Knoxville in Knoxville, Tennessee.",
    alt:
      "Service work on a ChargePoint Express 250 DC fast charger at Land Rover Knoxville in Tennessee.",
  },
  "range-rover-knoxville-tn-cpe250-dc-fast-charger-service-2-03-25.jpg": {
    caption:
      "ChargePoint Express 250 service at Land Rover Knoxville in Knoxville, Tennessee.",
    alt:
      "Service work on a ChargePoint Express 250 DC fast charger at Land Rover Knoxville in Tennessee.",
  },
  "southall-farms-franklin-tn-ct4000-ev-charger-service-call-05-25.jpg": {
    caption:
      "ChargePoint CT4000 service call at Southall Farm & Inn in Franklin, Tennessee.",
    alt:
      "ChargePoint CT4000 Level 2 EV charger service at Southall Farm & Inn in Franklin, Tennessee.",
  },
  "sungoldpower-12k-off-grid-solar-battery-microgrid-cabin-homestead-1.jpg": {
    caption:
      "SunGoldPower 12kW off-grid solar and 20.48kWh battery microgrid for residential backup and independent power.",
    alt:
      "SunGoldPower 12kW off-grid solar and 20.48kWh lithium battery microgrid system.",
  },
  "sungoldpower-12k-off-grid-solar-battery-microgrid-cabin-homestead-2.jpg": {
    caption:
      "Ground-mounted SunGoldPower solar array supporting an off-grid cabin and battery microgrid system.",
    alt: "Ground-mounted solar panel array beside an off-grid cabin and homestead.",
  },
  "tesla-wall-charger-nema-weather-box-install-residential.jpg": {
    caption: "Residential Tesla Wall Connector with weatherproof enclosure.",
    alt:
      "Tesla Wall Connector installed in a weatherproof NEMA enclosure at a residential property.",
  },
  "toyota-nashville-tn-chargepoint-cpe250-install-3-03-23.jpg": {
    caption:
      "ChargePoint CPE250 installation at Toyota Nashville — project photo 3.",
    alt:
      "ChargePoint CPE250 DC fast charger installed at a Toyota dealership in Nashville, Tennessee.",
  },
  "toyota-nashville-tn-chargepoint-cpe250-install-4-03-23.jpg": {
    caption:
      "ChargePoint CPE250 installation at Toyota Nashville — project photo 4.",
    alt:
      "ChargePoint CPE250 DC fast charger installed at a Toyota dealership in Nashville, Tennessee.",
  },
  "toyota-nashville-tn-chargepoint-cpf50-install-5-03-23.jpg": {
    caption:
      "ChargePoint CPF50 EV charger installation at Toyota Nashville — project photo 5.",
    alt:
      "ChargePoint CPF50 Level 2 EV charger installed at a Toyota dealership in Nashville, Tennessee.",
  },
  "toyota-nashville-tn-chargepoint-ct4020-pedestal-install-1-03-23.jpg": {
    caption:
      "ChargePoint CT4020 Level 2 pedestal installation at Toyota Nashville — project photo 1.",
    alt:
      "ChargePoint CT4020 dual-output Level 2 EV charger installed at a Toyota dealership in Nashville, Tennessee.",
  },
  "toyota-nashville-tn-chargepoint-ct4020-pedestal-install-2-03-23.jpg": {
    caption:
      "ChargePoint CT4020 Level 2 pedestal installation at Toyota Nashville — project photo 2.",
    alt:
      "ChargePoint CT4020 dual-output Level 2 EV charger installed at a Toyota dealership in Nashville, Tennessee.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-battery-bank-stage-1-03-25.jpg": {
    caption:
      "Luck Reunion battery bank supporting a stage using OverDrive equipment.",
    alt: "OverDrive battery bank supporting a stage at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-battery-hotswap-1-03-25.jpg": {
    caption:
      "Luck Reunion battery hot-swap operation using OverDrive equipment.",
    alt: "OverDrive battery hot-swap operation at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-battery-microgrid-1-03-25.jpg": {
    caption:
      "Luck Reunion battery microgrid deployment using OverDrive equipment.",
    alt: "OverDrive battery microgrid deployment at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-battery-tour-bus-1-03-25.jpg": {
    caption:
      "Luck Reunion battery power supporting a tour bus using OverDrive equipment.",
    alt: "OverDrive battery power supporting a tour bus at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-solar-field-install-1-03-25.jpg": {
    caption: "Luck Reunion solar field installation using OverDrive equipment.",
    alt: "OverDrive solar field installation at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-solar-field-install-2-03-25.jpg": {
    caption: "Luck Reunion solar field installation using OverDrive equipment.",
    alt: "OverDrive solar field installation at Luck Reunion in Texas.",
  },
  "willie-nelson-luck-reunion-tx-overdrive-solar-stage-setup-1-03-25.jpg": {
    caption:
      "Luck Reunion solar-powered stage setup using OverDrive equipment.",
    alt: "OverDrive solar-powered stage setup at Luck Reunion in Texas.",
  },
  "toyota-thomasville-ga-chargepoint-service-01-23.jpg": {
    caption: "ChargePoint EV charger service at Toyota Thomasville, Georgia.",
    alt:
      "Technician servicing ChargePoint EV charging equipment at a Toyota dealership in Thomasville, Georgia.",
  },
};