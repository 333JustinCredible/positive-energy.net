...
2. Create a standalone Apps Script project and paste `integrations/google-apps-script/Code.gs` into `Code.gs`.
...
- The hidden honeypot, per-client/per-email rate limits, Apps Script rate limits, request IDs, and recent payload fingerprints provide practical spam and duplicate protection.
